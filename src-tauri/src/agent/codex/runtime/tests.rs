...
fn completed_execution_idle_runtime_is_recovered_without_a_claim() {
    ...
    db.execute(
        "UPDATE runtime_instances SET state='running' WHERE id=?1",
        [&id],
    )
    .unwrap();
    let manager =
        crate::agent::task_manager::AgentTaskManager::new(store.clone(), "unused.exe".into());
...
fn pool_retains_failed_job_owner_and_blocks_shutdown_until_evidence_commits() {
    ...
    assert!(
        block(pool.shutdown())
            .unwrap_err()
            .contains("CODEX_RUNTIME_EVIDENCE_PERSIST_FAILED")
    );
    unknown(&store, &id);
...
fn claimed_startup_retains_recovered_owner_before_failing_execution_write() {
    ...
    let job = open_job(&format!("Local\\SerenaDesktop.Codex.{id}")).unwrap();
    drop(runtime);
    assert!(
        block(manager.recover_startup())
            .unwrap_err()
            .contains("execution failure")
    );
    assert!(block(manager.runtime_pool.shutdown()).is_err());
    db.execute_batch("DROP TRIGGER fail_evidence; DROP TRIGGER fail_unknown;")
        .unwrap();
...
fn ownerless_quarantine_retries_named_job_recovery_after_failed_evidence_commit() {
    let dir = tempfile::tempdir().unwrap();
    let store = open(dir.path());
    let id = format!("ownerless-quarantine-{}", std::process::id());
    prepare(&store, &id);
    let db = Connection::open(dir.path().join("agent-state.db")).unwrap();
    ...
    pool.retain_failure(
        ...
        &id,
        RuntimeFailure {
            code: "CODEX_RUNTIME_WORKER_FAILED",
            message: "ownerless post-create".into(),
            runtime: None,
        },
    );
    assert!(block(pool.shutdown()).is_err());
    assert!(pool.check_workspace(dir.path().to_str().unwrap()).is_err());
    db.execute_batch("DROP TRIGGER fail_recovery_evidence")
        .unwrap();
    block(pool.shutdown()).unwrap();
...
fn owned_timeout_quarantines_workspace_until_job_termination_is_proven() {
    ...
    pool.retain_failure(
        ...
        RuntimeFailure {
            ...
            message: "injected bounded termination timeout".into(),
...
fn repeated_claimed_recovery_reuses_retained_owner_and_shutdown_converges_once() {
    ...
    let manager = crate::agent::task_manager::AgentTaskManager::new(
        store.clone(),
        dir.path().join("must-not-launch.exe"),
    );
    ...
    assert!(
        manager
            .runtime_pool
            .retains_runtime(dir.path().to_str().unwrap(), &id)
    );
