...
async fn rt05_finalizing_restart_retains_terminal_and_recovers_result_under_v04() {
    ...
    for terminal in ["completed", "failed", "interrupted"] {
        ...
        service
            ...
            .provider_event(
                "E1".into(),
                crate::agent::execution::state::Transition::ResumeRecovery(
                    crate::agent::execution::state::RecoveryBasis::LocalResolve {
                        diagnostic: "fixture result recovery".into(),
                    },
                ),
...
async fn explicit_random_runtime_attempt_prebind_crash_is_never_resumable() {
    for persisted_runtime in [false, true] {
        ...
        pending(&store, dir.path(), "E1").await;
        store
            .reserve_runtime_attempt("E1".into(), "R123-independent".into(), 2)
            .await
            .unwrap();
        ...
        assert!(
            !service
                .checked_operation(json!({"action":"resume_pending","executionId":"E1"}), None)
                .await["ok"]
                .as_bool()
                .unwrap()
        );
        ...
        let db = Connection::open(dir.path().join("agent-state.db")).unwrap();
        assert_eq!(
            count(&db, "runtime_instances"),
            i64::from(persisted_runtime)
        );
        ...
        assert!(
            db.execute("DELETE FROM execution_runtime_attempts", [])
                .is_err()
        );
    }
}
...
async fn old_unresolved_recovery_attempt_quarantines_before_any_new_recovery_connect() {
    ...
    store
        .reserve_recovery_attempt("E1".into(), "recovery-R2".into(), 6)
        .await
        .unwrap();
    ...
    assert_eq!(
        count(
            &Connection::open(dir.path().join("agent-state.db")).unwrap(),
    ...
    assert_eq!(
        service
            .manager
            .runtime_pool
            .check_workspace(dir.path().to_str().unwrap())
            .unwrap_err(),
