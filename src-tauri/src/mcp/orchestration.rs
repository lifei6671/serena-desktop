...
pub fn descriptors() -> Vec<Tool> {
    ...
    [
        ...
        ("agent_execute", "【做什么】\n执行需要自主代码修改、工程分析和多步骤迭代的复合编码任务。\n\n【什么时候使用】\n方案已明确，但仍需 Agent 阅读代码、修改实现、根据中间结果调整并完成验证时使用；若只是运行已确定的命令，优先使用 command_execute。\n\n【关键约束】\nstart 必须显式携带 workspaceId，并由 Registry 解析后冻结；WorkRun 仅校验其一致性。continue 创建新 Execution，可复用 Thread；context 传递 Host 验证的 path+sha256 引用。start/continue 重试保留原 requestKey 和请求；cancel 取消指定 Execution，resume_pending 仅显式恢复允许首次派发的原 Execution。command_execute 能完成的确定性命令不应转交 Agent。", schema::<AgentExecute>(), super::registry::agent_output_schema(), false, true),
