...
fn wrapped_network_causes_only_yield_fixed_categories_and_safe_tls_details() {
    for (message, category, detail) in [
        ...
        (
            "invalid peer certificate: UnknownIssuer PRIVATE_DETAIL",
            "tls",
            Some("unknown_issuer"),
        ),
        (
            "invalid peer certificate: NotValidForName PRIVATE_HOST",
            "tls",
            Some("hostname"),
        ),
        (
            "certificate revocation failed PRIVATE_DETAIL",
            "tls",
            Some("revocation"),
        ),
        (
            "TLS handshake unexpected EOF PRIVATE_DETAIL",
            "tls",
            Some("eof"),
        ),
        (
            "AcquireCredentialsHandle failed: SEC_E_NO_CREDENTIALS PRIVATE_DETAIL",
            "tls",
            Some("credential"),
        ),
        (
            "proxy authentication failed PRIVATE_PASSWORD",
            "proxy",
            None,
        ),
    ] {
        ...
        if let Some(detail) = detail {
            let evidence = ProbeStage::new("oauth_metadata", "fixture.example").fail_with_detail(
                "tls",
                None,
                Some(tls_subtype(&error)),
            );
            assert!(
                evidence.contains(&format!("category=tls detail={detail}")),
                "{evidence}"
            );
